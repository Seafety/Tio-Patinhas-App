...
const GraficoLucro = () => {
  ...
  useEffect(() => {
    ...
    if (lucroAleatorio >= metaLucro) {
      ...
      setMensagemMeta(`Faltam ${metaLucro - lucroAleatorio}% para atingir a meta!`);
    }
  }, []);
  ...
  return (
    <div className={styles.graficoLucro}>
      ...
      <div className={styles.meta}>
        <p><strong>Meta:</strong> 80% de lucro</p>
        <p>{mensagemMeta}</p>
        <p><strong>Status:</strong> {metaAlcancada ? 'Meta atingida!' : 'Meta não atingida'}</p>
      </div>

      {metaAlcancada ? (
        <div className={styles.resumoMetaAlcancada}>
          <p>Continue assim! Você está no caminho certo para superar suas expectativas de crescimento.</p>
        </div>
      ) : (
        <div className={styles.resumoMetaPendente}>
          <p>Atingir a meta é possível! Revise seus investimentos e foque em estratégias de otimização para melhorar os resultados.</p>
        </div>
      )}
    </div>
  );
