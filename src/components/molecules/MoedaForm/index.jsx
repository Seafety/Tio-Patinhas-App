...
const MoedaForm = ({ tipo, onAction }) => {
  ...
  useEffect(() => {
    ...
    if (moedaSelecionada.preco) {
      ...
      setPrecoCalculado(new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(calculo));
    } else {
  ...
  const handleAction = () => {
    ...
    if (valorNumerico > 0 && moedaSelecionada.preco > 0) {
      onAction(moedaSelecionada, valorNumerico, parseFloat(precoCalculado.replace('R$', '').replace('.', '').replace(',', '.')));
    } else {
      alert('Por favor, selecione uma moeda válida e insira uma quantidade maior que zero.');
    }
  };
  ...
  return (
    <div className={styles.moedaForm}>
      ...
      <button
        className={tipo === 'Comprar' ? styles.comprarButton : styles.venderButton}
