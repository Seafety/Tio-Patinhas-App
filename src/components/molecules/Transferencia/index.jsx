...
const Transferencia = ({ onTransferir }) => {
  ...
  const handleTransferir = () => {
    ...
    const saldoEmpresa =
      saldos.empresas[empresaSelecionada][moedaSelecionada.symbol];

    if (valorNumerico > saldoEmpresa) {
      setErro(
        `Saldo insuficiente. A empresa ${empresaSelecionada} possui apenas ${saldoEmpresa} ${moedaSelecionada.symbol}.`
      );
      return;
    }

    const novosSaldos = { ...saldos };
    novosSaldos.empresas[empresaSelecionada][moedaSelecionada.symbol] -=
      valorNumerico;
    novosSaldos.contas[contaSelecionada][moedaSelecionada.symbol] +=
      valorNumerico;
    ...
    setMensagem(
      ...
      } de ${empresaSelecionada} para ${contaSelecionada} realizada com sucesso!`
    );
    setErro("");
    onTransferir(
      moedaSelecionada,
  ...
  return (
    <div className={styles.transferenciaForm}>
      ...
      <div className={styles.inputGroup}>
        <label style={{ backgroundColor: "rgba(122, 122, 122, 0.70)" }}>
          Moeda
        </label>
        ...
        <div className={styles.available}>
          Disponível na Empresa {empresaSelecionada}:{" "}
          {saldos.empresas[empresaSelecionada][
            moedaSelecionada.symbol
