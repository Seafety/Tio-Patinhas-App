        ...
        <div className={styles.cryptoBoxContent}>
          {cryptoData.popularCoins.map((coin, index) => (
            <div key={index} className={styles.coinItem}>
              <img src={iconMap[coin.icon]} alt={coin.name} className={styles.coinIcon} />
              <span className={styles.coinName}>{coin.name}</span>
              <span className={styles.coinSymbol}>{coin.symbol}</span>
              <span className={styles.coinPrice}>{coin.price}</span>
              <span className={`${styles.coinChange} ${parseFloat(coin.change) > 0 ? styles.positive : styles.negative}`}>
                {coin.change}
              </span>
            </div>
          ))}
